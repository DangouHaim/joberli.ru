<?php
class Olam_Download_Item_Video extends FES_Field {
	...
	public $supports = array(
		...
		'template'    => 'download_item_video_id',
		'title'       => 'Download Video Upload',
		'phoenix'    => false,
	);

	/** @var array Characteristics are things that can change from field to field of the same field type. Like the placeholder between two download_item_video_id fields. Stored in db. */
	public $characteristics = array(
		'name'        => 'download_item_video_id',
		'template'    => 'download_item_video_id',
		'public'      => true,
		'required'    => false,
		'label'       => '',
	...
	public function set_title() {
		$title = _x( 'Download video', 'FES Field title translation', 'olam-edd-fes-meta-fields' );
	...
	public function render_field_admin( $user_id = -2, $readonly = -2 ) {
		...
		$user_id   = apply_filters( 'fes_render_download_item_video_id_field_user_id_admin', $user_id, $this->id );
		$readonly  = apply_filters( 'fes_render_download_item_video_id_field_readonly_admin', $readonly, $user_id, $this->id );
		...
		ob_start(); ?>
			<div class="fes-fields">
				 <table class="<?php echo sanitize_key( $this->name() ); ?>">
					<thead>
						<tr>
							<td class="fes-file-column" colspan="2"><?php esc_html_e( 'Download item thumb URL', 'olam-edd-fes-meta-fields' ); ?></td>
							<?php if ( fes_is_admin() ) { ?>
							...
							foreach ( $uploaded_items as $index => $attach_id ) {
								...
								<tr class="fes-single-variation">
									 ...
									 <td class="fes-url-choose-row" width="1%">
												<a href="#" class="edd-submit button upload_file_button" data-choose="<?php esc_attr_e( 'Choose file', 'olam-edd-fes-meta-fields' ); ?>" data-update="<?php esc_attr_e( 'Insert download item thumb URL', 'olam-edd-fes-meta-fields' ); ?>">
	...
	public function render_field_frontend( $user_id = -2, $readonly = -2 ) {
		...
		$user_id   = apply_filters( 'fes_render_download_item_video_id_field_user_id_frontend', $user_id, $this->id );
		$readonly  = apply_filters( 'fes_render_download_item_video_id_field_readonly_frontend', $readonly, $user_id, $this->id );
		...
		ob_start(); ?>
			<div class="fes-fields">
				 <table class="<?php echo sanitize_key( $this->name() ); ?>">
					<thead>
						<tr>
							<th class="fes-file-column" colspan="2"><?php esc_html_e( 'Download item thumb URL', 'olam-edd-fes-meta-fields' ); ?></th>
							<?php if ( fes_is_admin() ) { ?>
							...
							foreach ( $uploaded_items as $index => $attach_id ) {
								...
								<tr class="fes-single-variation">
									 ...
									 <td class="fes-url-choose-row">
												<a href="#" class="edd-submit button upload_file_button" data-choose="<?php esc_attr_e( 'Choose file', 'olam-edd-fes-meta-fields' ); ?>" data-update="<?php esc_attr_e( 'Insert download item video URL', 'olam-edd-fes-meta-fields' ); ?>">
												<?php echo str_replace( ' ', '&nbsp;', esc_html__( 'Choose file', 'olam-edd-fes-meta-fields' ) ); ?></a>
									 </td>
									 <?php if ( empty( $this->characteristics['single'] ) || $this->characteristics['single'] !== 'yes' ) { ?>
									<?php } ?>
					...
					<tfoot>
						<tr>
							<th colspan="5">
								<a href="#" class="edd-submit button insert-file-row" id="<?php echo sanitize_key( $this->name() ); ?>"><?php esc_html_e( 'Add TrFile', 'olam-edd-fes-meta-fields' ); ?></a>
							</th>
						</tr>
					</tfoot>
					<?php endif; ?>
					...
					<td class="fes-display-field-values">
						...
						if ( is_array( $value ) ) {
							foreach ( $value as $attachment_id ) {
								$uploads[] = wp_get_attachment_link( $attachment_id, 'video', false, true );
							}
	...
	public function save_field_admin( $save_id = -2, $value = '', $user_id = -2 ) {
		...
		if ( $this->type === 'user' ) {
			...
			update_post_meta( $save_id,'download_item_video', $url);
		} else {
			// todo: do action
		}
	...
	public function get_field_value_admin( $save_id = -2, $user_id = -2, $public = -2 ) {
		...
		if ( $this->type === 'user' ) {
			...
			$value = apply_filters( 'fes_get_custom_download_item_video_id_value_admin', $save_id, $user_id, $public );
		}
	...
	public function get_field_value_frontend( $save_id = -2, $user_id = -2, $public = -2 ) {
		...
		if ( $this->type === 'user' ) {
			...
			$value = apply_filters( 'fes_get_custom_download_item_video_id_value_frontend', $save_id, $user_id, $public );
		}
